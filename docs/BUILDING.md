...
Expected Linux-side release files:
...
Depending on available local packaging tools, the release directory may also contain:
...
Windows GUI release assets should be built on Windows:
...
Wails v2 Linux GUI builds use WebKitGTK. Some older distributions provide `webkit2gtk-4.0`; newer distributions often provide only `webkit2gtk-4.1`.

`build.sh` automatically detects this through `pkg-config`:

- `webkit2gtk-4.1` found → use `-tags webkit2_41`
- `webkit2gtk-4.0` found → no extra Wails tag

Manual override:

```bash
WAILS_WEBKIT_TAG=webkit2_41 ./build.sh linux
```

Compatibility alias:

```bash
WAILS_TAGS=webkit2_41 ./build.sh linux
```
